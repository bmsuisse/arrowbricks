...
- `rust/arrowbricks_core/` -- PyO3+arrow-rs reimplementation of the hot path above; this is arrowbricks' committed future implementation, not a side experiment. Full feature parity reached: lazy fetchmany/fetchall (execute() + ResultSet), token_provider (sync/async callback), JSON result format (execute_json), volume-file ops (upload_volume_file/delete_volume_file), execute_streamed/fetchall_arrow_streamed heartbeats (HEARTBEAT singleton, matches _streaming.py's await_with_heartbeat). Not wired into the Python package's build yet and not published to PyPI -- that cutover (swap the build to this crate behind the same public API, delete the Python implementation) is the next step, not a permanent side-by-side option. See its own `README.md`.
