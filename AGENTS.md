...
- `src/arrowbricks/_streaming.py` -- `HEARTBEAT`/`QueryTimeout`/`await_with_heartbeat` (pure Python, shared by `cursor.py`), `write_ipc_stream` (delegates to `._core.write_ipc_stream` -- always uncompressed, see below), and `stream_query_json` -- which delegates its submit/poll/fetch/reorder/decode/NDJSON-encode *entirely* to `._core.Client.stream_ndjson_lines`; this module only forwards already-formatted lines.
