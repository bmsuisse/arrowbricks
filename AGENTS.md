...
- `rust/arrowbricks_core/` -- PyO3+arrow-rs reimplementation of the hot path above; this is arrowbricks' committed future implementation, not a side experiment. Replaces the Python package's build once it reaches feature parity, at which point the Python implementation gets deleted, not kept as a fallback. Ported so far: lazy fetchmany/fetchall (execute() + ResultSet), token_provider (sync/async callback), JSON result format (execute_json), volume-file ops (upload_volume_file/delete_volume_file). One gap left: execute_streamed/heartbeats. Not wired in and not published until that closes. See its own `README.md`.
